...
export class SessionBuilder {
  ...
  startSessionAsInitiator = async (
    ...
    function assert32Bytes(buf: ArrayBuffer, label: string) {
      if (buf.byteLength !== 32) {
        throw new Error(`${label} must be 32 bytes, got ${buf.byteLength}`)
      }
    }
    assert32Bytes(ecRes[0], "DH1")
    assert32Bytes(ecRes[1], "DH2")
    assert32Bytes(ecRes[2], "DH3")
  ...
  async processV3(record: SessionRecord, message: PreKeyWhisperMessage): Promise<number | void> {
    ...
    const identityKey = uint8ArrayToArrayBuffer(message.identityKey);
    const trusted = await this.storage.isTrustedIdentity(
      this.remoteAddress.name,
      identityKey,
      Direction.RECEIVING
